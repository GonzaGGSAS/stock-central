/**
 * STOCK CENTRAL v2 - Backend para Tiendanube
 */
...
app.post('/webhook/order', async (req, res) => {
  ...
  try {
    ...
    for (const item of order.products || []) {
      ...
      if (!foundProd || !foundVar) continue;
      const newStock = event === 'order/paid' ? Math.max(0, foundVar.stock - quantity) : foundVar.stock + quantity;
      db.get('productos').find({ id: foundProd.id }).get('variantes').find({ id: foundVar.id })
        .assign({ stock: newStock }).get('log').unshift({
          ts: new Date().toISOString(), action: event === 'order/paid' ? 'sale' : 'return',
          order_id: String(order_id), delta: event === 'order/paid' ? -quantity : +quantity, stock: newStock
        }).write();
...
app.listen(PORT, () => {
  console.log(`Stock Central v2 en puerto ${PORT}`);
